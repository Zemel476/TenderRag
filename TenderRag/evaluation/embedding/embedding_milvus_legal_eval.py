# -*- coding: utf-8 -*-
"""
embedding_milvus_legal_eval.py
Milvus 向量库粗排+精排召回率评估 —— Embedding vs BM25 vs Hybrid(RRF)
...
def do_rerank(query: str, documents: list[str], coarse_ids: list[str],
    ...
    try:
        resp = text_rerank(
            ...
            documents=documents,
            top_n=RERANK_TOP_N,
            instruct="从候选法律文档中检索与查询问题最相关的条款"
        )
...
def main():
    ...
    try:
        if not repository.collection_exists(collection_name):
            print(f"集合 '{collection_name}' 不存在，请先运行 vector_index_legal.py 构建索引")
            return
        ...
        print(f"\n{'='*85}")
        print(f"召回率对比 (k={RRF_K}, w_emb={RRF_W_EMB})")
        print(f"{'='*85}")
